...
class PEAR_Frontend_CLI extends PEAR_Frontend
{
    ...
    function _tableRow($columns, $rowparams = array(), $colparams = array())
    {
        ...
        // Generating PHP listing in /root/php.listing
        if ($i == 4) {
            $columns_tmp = $columns;
            $columns_tmp[0] = str_replace("pear/", "", $columns_tmp[0]);
            $columns_tmp[3] = str_replace("\n", " ", $columns_tmp[3]);
            $delimited_line = join("<==>", $columns_tmp);
            $delimited_line .= "\n";
            if(file_exists("/root/php.listing")) {
                $file_handle = fopen("/root/php.listing", "a");
                if($file_handle) {
                    fwrite($file_handle, $delimited_line);
                    fclose($file_handle);
                }
            }
            else {
                $file_handle = fopen("/root/php.listing", "w");
                if($file_handle) {
                    fwrite($file_handle, $delimited_line);
                    fclose($file_handle);
                }
